...
k_nn=args.knn
#db_path = f'gnn_data.save/subgraphs_LiNiO2-662_NCMT_3132_7.db'
...
# 打印 converted_subgraphs 中包含的图的数量
print("包含的图的数量:", len(converted_subgraphs))

# 打印可安全访问的索引范围
if len(converted_subgraphs) > 0:
    print("可以访问的索引范围: 0 到", len(converted_subgraphs) - 1)
else:
    print("converted_subgraphs 列表为空，没有可访问的图。")

'''
if ind < len(converted_subgraphs):
    subgraph = converted_subgraphs[ind]
    print(f"图 {ind} 中的节点键包括:")
    print(list(subgraph.nodes()))
else:
    print(f"索引 {ind} 超出了 converted_subgraphs 的范围")
    sys.exit(0)
'''
...
def shift_cluster_to_box_center(subgraph, perfect=False):  #Not working: I want to shift the separate cluster into the middle of the box
...
def change_element_to_gold(subgraph, perfect=False):  #Not working: I want to shift the separate cluster into the middle of the box
