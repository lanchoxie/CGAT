    ...
    for atom in boundary_atoms:
        ...
        for neighbor in graph.neighbors(atom):
            if graph.edges[atom, neighbor]['edge_type'] == 'original':
                #if frozenset([atom,neighbor]) not in atom_ind_dir_pair:
                    #atom_ind_dir_pair[frozenset([atom, neighbor])]=set()
                if (atom,neighbor) not in atom_ind_dir_pair:
                    atom_ind_dir_pair[(atom, neighbor)]=set()
...
def connect_graphs(G_images, all_directs):
...
def handle_other_manual_edges(G_expand,expand_vecs,spe_edge_attr):
    ...
    for u,v,data in G_expand.edges(data=True):
        if data['edge_type']=='li_ni_edge': 
            count_nm+=1
            if 'delta_E' in data and data['delta_E']!=None:
                count_av_bf.append((u,v))
    ...
    for key,data in bond_break.items():
        ...
        if frozenset([closest_av_node,ni_atom_orig]) in new_add_bond:
            print("=================ERROR",frozenset([closest_av_node,ni_atom_orig]),"in", new_add_bond)
            continue
        ...
        for key in matching_keys:
            if new_add_bond[key]==frozenset([li_atom_ft,G_expand.nodes[ni_atom_orig]['feature'][-2]]):
                if frozenset([closest_av_node,ni_atom_orig])!=key:
                    ...
                    print("===============ERROR",new_add_bond[key],"=",frozenset([li_atom_ft,G_expand.nodes[ni_atom_orig]['feature'][-2]]))
                    next_loop=1
        if next_loop:
            continue
        '''
    ...
    for key,data in bond_break.items():
        ...
        li_atom_av_pack=[[av_node,calculate_real_distance(np.array(cartesian_to_fractional(G_expand.nodes[ni_atom_orig]['feature'][-5:-2], orig_lattice_vectors)) , np.array(cartesian_to_fractional(G_expand.nodes[av_node]['feature'][-5:-2], orig_lattice_vectors)) )] for av_node in li_nodes[li_atom_ft]]
        #li_atom_av_pack_car=[[av_node,calculate_real_distance(np.array(G_expand.nodes[ni_atom_orig]['feature'][-5:-2]) , np.array(G_expand.nodes[av_node]['feature'][-5:-2]))] for av_node in li_nodes[li_atom_ft]]
        closest_av_node = min(li_atom_av_pack, key=lambda x: x[1])[0]
        closest_dis = min(li_atom_av_pack, key=lambda x: x[1])[1]
        closest_dis_car = [i[1] for i in li_atom_av_pack_car if i[0]==closest_av_node]
    ...
    for u,v,data in G_expand.edges(data=True):
        if data['edge_type']=='li_ni_edge': 
            if 'delta_E' in data and data['delta_E']!=None:
                count_av_af.append((u,v))

    print("total lini:",count_nm,"change lini:", len(bond_break), "act_change_lini",add_count,"lini_av_before:",len(count_av_bf),"lini_av_after:",len(count_av_af))
    print("lini_av_before:",count_av_bf,"\nlini_av_after:",count_av_af)
    return G_expand
...
graph_test = get_graph_by_name(db_file, "LiNiO2-331_NCMT_3123_6")
all_graphs=[graph_test]
print(all_graphs[0])
...
boundary_directions = find_boundary_direction_by_fractional(boundary_atoms, graph_loc, lattice_vectors)
#print("\nLV",lattice_vectors,"\nFT",[attr for node,attr in graph_test.nodes(data=True)][0]['feature'])
write_xsf("all_atoms_u.xsf", graph_loc, lattice_vectors)
write_xsf("boundary_atoms_u.xsf", graph_loc, lattice_vectors, boundary_atoms)
write_xsf("boundary_atoms_u_marked.xsf", graph_loc, lattice_vectors, boundary_atoms, boundary_directions)
...
if not further_expand and expand_dir:
    ...
    print("+*"*60)
    '''
    G_expand,exp_vecs = connect_graphs(G_images, expand_dir)
    print("="*60)
    print("="*60)
    print("="*60)
    ...
    boundary_directions = find_boundary_direction_by_fractional(boundary_atoms, graph_loc2, lattice_vectors)
...
write_xsf("boundary_atoms.xsf", graph_loc, lattice_vectors, boundary_atoms)
write_xsf("boundary_atoms_marked.xsf", graph_loc, lattice_vectors, boundary_atoms, boundary_directions)


all_graphs2=[graph_loc]
print("Creating whole graphs...")
for k in range(2,3):
    print(f"\nextract subgraphs k neighbor:{k}")
    ...
    for i,v in enumerate(output_subgraphs_train):
        if len(v.nodes)!=38:
            print(i,len(v.nodes))
            for x,y,data in v.edges(data=True):
                if data['edge_type']=='li_ni_edge': 
                    print("lini:",x+1,y+1)
