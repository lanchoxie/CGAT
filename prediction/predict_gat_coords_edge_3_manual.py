...
# 获取当前脚本的父目录的路径（即CGAT_train目录）
parent_directory = os.path.dirname(current_script_path)
project_root = os.path.dirname(parent_directory)

# 将CGAT_train目录添加到sys.path中
sys.path.append(project_root)
from train_gat_coords_edge_3 import networkx_to_pyg_graph,CustomGATConv, GAT
...
db_dir=current_dir+"/"+args.database.strip("/")+'/' if "/" in args.database else current_dir+"/"
db_path = f'{db_dir}/subgraphs_k_neighbor_{dataset_k_hop}_gnn_53d_feature_predict.db'
predict_path=f"{current_dir}/prediction_results"
...
predict_num=6  #0 for all prediction
...
def networkx_to_pyg_graph_old(subgraph,k_hop_attention,opt_dis,exchange_skip=True):
...
def process_subgraph(subgraph,k_hop_attention,opt_dis):
    # 假设networkx_to_pyg_graph是你的函数，用于转换图的格式
    pyg_graph = networkx_to_pyg_graph(subgraph,k_hop_attention,opt_dis,exchange_skip=False)
    #if pyg_graph:
    #    print("1")
    return pyg_graph
...
def convert_graphs1(graphs, k_hop_attention, opt_dis):
    with ProcessPoolExecutor(max_workers=2) as executor:
        # 创建future到索引的映射
        future_to_index = {executor.submit(process_subgraph, graph, k_hop_attention, opt_dis): i for i, graph in enumerate(graphs)}
        converted_graphs = [None] * len(graphs)
        total_done = 0
        total_graphs = len(graphs)

        # 检查所有任务是否完成
        while total_done < total_graphs:
            time.sleep(0.1)  # 等待一小段时间再检查，减少CPU使用率
            for future in list(future_to_index):
                if future.done():
                    index = future_to_index[future]
                    try:
                        result = future.result()
                        converted_graphs[index] = result
                        total_done += 1
                        print(f"Graph {index} processing completed successfully.")
                        print_progress(total_done, total_graphs)  # 更新进度条
                    except Exception as exc:
                        print(f"Graph processing failed at index {index} with an exception: {exc}")
                    del future_to_index[future]  # 从字典中移除这个future

    successful_transforms = sum(1 for g in converted_graphs if g is not None)
    print(f"Total successful transformations: {successful_transforms}/{total_graphs}")
    return [graph for graph in converted_graphs if graph is not None]

def convert_graphs(graphs,k_hop_attention,opt_dis):
    with ProcessPoolExecutor() as executor:
        # 创建future到索引的映射
        futures = {executor.submit(process_subgraph, subgraph,k_hop_attention,opt_dis): i for i, subgraph in enumerate(graphs)}
...
class CustomGATConv_old(MessagePassing):
...
def extract_params_from_path(model_path):
    ...
    opt_match = re.search(r'opt_([Tt]rue|[Ff]alse)', model_path)
    #opt_match = re.search(r'opt_(\w+)[._]', model_path)
...
def predict_from_model(model_path, predict_graphs,batch_size):
    ...
    predict_dataset = convert_graphs(predict_graphs,k_hop_attention,opt_dis)
    print()
    print("S",len(predict_dataset))
...
for ind,i in enumerate(model_paths):
    ...
    print(f"Memory used before the function call: {mem_usage_before} MB")
    print(f"Memory used after the function call: {mem_usage_after} MB")
...
def run_model_prediction(model_path, predict_graphs, batch_size):
    print(f"Starting prediction for model: {model_path}")
...
def worker(model_path, predict_graphs, batch_size, predictions_whole, model_av):
    predictions_i = run_model_prediction(model_path, predict_graphs, batch_size)
    if predictions_i is not None:
        predictions_whole.append(predictions_i)
        model_av.append(model_path)

dataset_infos = ext_subgraph_infos(predict_graphs)
manager = multiprocessing.Manager()
predictions_whole = manager.list()
model_av = manager.list()

for ind, model_path in enumerate(model_paths):
    print(f"Process on Model : {ind+1}/{len(model_paths)}")
    # 创建并启动一个新的子进程
    process = Process(target=worker, args=(model_path, predict_graphs, batch_size, predictions_whole, model_av))
    process.start()
    process.join()  # 等待这个子进程结束后再继续

print("All models processed.")
predictions_whole = list(predictions_whole)
model_av = list(model_av)
...
output_data.append('Index\tStructure\tatom1\tatom2\t'+"\t".join([f"Pre_of_Model_{j+1}" for j in range(len(predictions_whole))])+"\tSTD\n")
