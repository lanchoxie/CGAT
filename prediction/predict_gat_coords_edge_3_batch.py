...
parser.add_argument('opt', type=str2bool, default=False, help='True for use optimized str model, False for using lattice size model.')
parser.add_argument('target_dir',type=str, help='Enter the target dir generated by CGAT_train/batch_nn_train.py')
...
predict_num=100  #0 for all prediction

opt_dis=args.opt

model_paths = args.target_dir.strip("/")
# 读取NN_batch_state文件
if not os.path.isfile(os.path.join(model_paths, "NN_batch_state")):
    print(os.path.join(model_paths, "NN_batch_state"),"Not found!")
    sys.exit(0)
df = pd.read_csv(os.path.join(model_paths, "NN_batch_state"), delimiter='\t', header=0)
...
for index, row in df.iterrows():
    ...
    # 处理True R2相关的文件
    if not pd.isnull(row['True_R2']):
        # 构建目录的可能前缀
        true_r2_prefix = str(row['True_R2'])
        # 遍历SEED目录下所有子目录
        for subdir in os.listdir(os.path.join(model_paths, model_name)):
            if subdir.startswith(true_r2_prefix):
                true_subdir = os.path.join(model_paths, model_name, subdir)
                # 如果该目录存在，则遍历目录下的文件
                if os.path.isdir(true_subdir):
                    for file in os.listdir(true_subdir):
                        if file.startswith('model'):
                            true_models.append(os.path.join(true_subdir, file))

    # 处理False R2相关的文件
    if not pd.isnull(row['False_R2']):
        # 构建目录的可能前缀
        false_r2_prefix = str(row['False_R2'])
        # 遍历SEED目录下所有子目录
        for subdir in os.listdir(os.path.join(model_paths, model_name)):
            if subdir.startswith(false_r2_prefix):
                false_subdir = os.path.join(model_paths, model_name, subdir)
                # 如果该目录存在，则遍历目录下的文件
                if os.path.isdir(false_subdir):
                    for file in os.listdir(false_subdir):
                        if file.startswith('model'):
                            false_models.append(os.path.join(false_subdir, file))


model_paths = true_models if opt_dis else false_models
...
def networkx_to_pyg_graph_old(subgraph,k_hop_attention):
...
def extract_params_from_path(model_path):
    ...
    #opt_match = re.search(r'opt_(\w+)_katt', model_path)
...
f1=open(f"{predict_path}/output_prd_num_{predict_num}_std_crt_{critical_std}_opt_{opt_dis}.csv","w+")
for i in output_data:
    f1.writelines(i)
f1.close()
print("$"*60)
print("#"*30+"Below Files Has Created"+"#"*30)
print(f"{predict_path}/output_prd_num_{predict_num}_std_crt_{critical_std}_opt_{opt_dis}.csv created!(Store all the predictions)")
...
if cal_std: 
    ...
    f1=open(f"{predict_path}/Top_{n_top}_prd_num_{predict_num}_std_crt_{critical_std}_diverse_datas_opt_{opt_dis}.csv","w+")
    for i in output_data_2:
        f1.writelines(i)
    f1.close()
    print(f"{predict_path}/Top_{n_top}_prd_num_{predict_num}_std_crt_{critical_std}_diverse_datas_opt_{opt_dis}.csv created!(Store {n_top} un-familiar datas extract by QBC,Query By Committee Method)")
elif not cal_std:
    model_infos=[]
    for i, model_path in enumerate(model_av):
        params = extract_params_from_path(model_path)
...
if args.fig:
    ...
    fig_name_pred=f'{predict_path}/value_prediction_Query-By-Committee_prd_num_{predict_num}_std_crt_{crit_str}_opt_{opt_dis}_10_6.png'
    fig_name_std=f'{predict_path}/std_of_predictions_prd_num_{predict_num}_std_crt_{crit_str}_opt_{opt_dis}_10_6.png'
    dpi_fig=1200 
