...
k_nn=args.knn
db_path = f'gnn_data.save/subgraphs_LiNiO2-662_NCMT_3132_7_k_{k_nn}.db'
...
def extract_lattice_and_coords(graph):
    ...
    for node, data in graph.nodes(data=True):
        ...
        if not perf_lat:  # 假设所有节点的晶格信息相同，只需提取一次
            perf_lat.extend(features[-14:-5])  # 提取优化后晶格信息
        
        # 提取坐标信息
        perf_coords.append(features[-5:-2])  # 提取优化后坐标信息
        # 提取原子索引
        index.append(features[-2])

    # 打包，然后根据 index 排序
    packed_list = list(zip(index, perf_coords, perf_element))
    packed_list.sort(key=lambda x: x[0])  # 假设index是可排序的
    
    # 解包
    index, perf_coords,perf_element= zip(*packed_list)
...
def shift_cluster_to_box_center(subgraph, perfect=False):  #Not woring: I want to shift the separate cluster into the middle of the box
    ...
    first_node_id = next(iter(subgraph[ind]))
    features=subgraph.nodes[first_node_id]['feature']
    lat_list=[]
...
def change_element_to_gold(subgraph):  #Not woring: I want to shift the separate cluster into the middle of the box
      
    first_node_id = next(iter(subgraph[ind]))
    features=subgraph.nodes[first_node_id]['feature']
    lat_list=[]
    if not lat_list:  # 假设所有节点的晶格信息相同，只需提取一次
        lat_list.extend(features[-14:-5])  # 提取晶格信息
    lat=np.array(lat_list).reshape((3,3))
    #perf_coords.append(features[-17:-14] if perfect else features[-5:-2])  # 提取坐标信息
    elements_list = ["Li", "O", "Mg", "Al", "Ti", "V", "Mn", "Co", "Ni", "Zr"]
    ...
    u_coords = subgraph.nodes[u]['feature'][-5:-2]
    v_coords = subgraph.nodes[v]['feature'][-5:-2]
    u_element_index = np.argmax(subgraph.nodes[u]['feature'][:10])  # 假设前10维是独热编码
    u_element = elements_list[u_element_index]  # 获取元素种类
    v_element_index = np.argmax(subgraph.nodes[v]['feature'][:10])  # 假设前10维是独热编码
    v_element = elements_list[v_element_index]  # 获取元素种类
    ...
    print("*"*60)
    coords = [subgraph.nodes[node]['feature'][-5:-2] for node in subgraph.nodes]
...
change_ele_coords,change_ele_eles=change_element_to_gold(subgraph)
...
if create_figs:
    ...
    if center1 is not None and center2 is not None:
        ...
        pos,lables = calculate_node_positions_for_centers1(subgraph, center1, center2, k=k_nn+5)
        #print(pos,lables)
        draw_subgraph_with_colored_nodes_and_energy(subgraph, pos, center1, center2, color_map)
