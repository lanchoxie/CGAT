...
predict_path="{current_dir}/prediction_results"
...
def networkx_to_pyg_graph_old(subgraph,k_hop_attention,opt):
...
def process_subgraph(subgraph,k_hop_attention,opt):
...
def convert_graphs(graphs,k_hop_attention,opt):
    with ProcessPoolExecutor() as executor:
        # 创建future到索引的映射
        futures = {executor.submit(process_subgraph, subgraph,k_hop_attention,opt): i for i, subgraph in enumerate(graphs)}
...
def predict_from_model(model_path, predict_graphs,batch_size):
    ...
    predict_dataset = convert_graphs(predict_graphs,k_hop_attention,opt)
    print()
...
if args.fig:
    ...
    for i, predictions in enumerate(predictions_whole):
        ...
        plt.plot(range(len(predictions)), predictions, label=f'Model {i+1} R2:{model_infos[i][0]} K:{model_infos[i][1]} H:{model_infos[i][2]} seed:{model_infos[i][4]} opt:{model_infos[i][3]}')
