...
#    print("Not here")
#os.chdir(dir)
# os.chdir(dir_check) 的调用应该在检查之前
os.chdir(dir_check)
read_dir=max(os.popen("pwd").read().split("\n"))

found_jobs = False  # 标志位，用来标记是否找到了作业

# 检查等待中的作业
if read_dir in working_file:
    found_jobs = True
    wait_index = working_file.index(read_dir)
    print("###Waiting job(s):")
    # 输出所有匹配的等待作业
    while wait_index < len(working_file) and working_file[wait_index] == read_dir:
        print(wait_node_list[wait_index])
        wait_index += 1

# 检查运行中的作业
if read_dir in running_file:
    found_jobs = True
    run_index = running_file.index(read_dir)
    print("###Running job(s):")
    # 输出所有匹配的运行作业
    while run_index < len(running_file) and running_file[run_index] == read_dir:
        print(run_node_list[run_index])
        run_index += 1
