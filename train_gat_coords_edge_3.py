...
def networkx_to_pyg_graph(subgraph,k_hop_attention,opt_dis,exchange_skip=True):
    ...
    for node in subgraph.nodes():
        if node not in [u, v]:
            k_neighbors = get_k_hop_neighbors(subgraph, node, k_nn)
            for neighbor in k_neighbors:
                if not subgraph.has_edge(node, neighbor):  # 避免重复添加已经存在的边
                    node_vec = subgraph.nodes[node]['feature'][range1_lat_coord:range2_lat_coord]
                    neighbor_vec = subgraph.nodes[neighbor]['feature'][range1_lat_coord:range2_lat_coord]
                    distance = calculate_minimum_distance(node_vec, neighbor_vec)
                    if distance != 0:
                        additional_edge_indices.append([node, neighbor])
                        additional_edge_distances.append([distance])
    '''
    k_neighbors={node:set() for node in subgraph.nodes()} 
    for node in subgraph.nodes():
        k_neighbors[node] = get_k_hop_neighbors(subgraph, node, k_nn)
    # 遍历每个节点
    ...
    for node in subgraph.nodes():
        # 使用最小堆找到前6个最近的邻居
        distances = []
        for neighbor in list(k_neighbors[node]):
            if not subgraph.has_edge(node, neighbor):  # 避免重复添加已经存在的边
                node_vec = subgraph.nodes[node]['feature'][range1_lat_coord:range2_lat_coord]
                neighbor_vec = subgraph.nodes[neighbor]['feature'][range1_lat_coord:range2_lat_coord]
                distance = calculate_minimum_distance(node_vec, neighbor_vec)
        ...
        # 选择距离最短的前6个邻居
        top_neighbors = heapq.nsmallest(6, distances)
  
        for distance, neighbor in top_neighbors:
            if not subgraph.has_edge(node, neighbor):
                additional_edge_indices.append([node, neighbor])
                additional_edge_distances.append([distance])
                #print(f"{node}-{elements_list[np.argmax(subgraph.nodes[node]['feature'][:10])]}-{neighbor}-{elements_list[np.argmax(subgraph.nodes[neighbor]['feature'][:10])]}:{distance}")
    '''

    # 检查是否找到了带有能量信息的边
    if exchange_energy is None and exchange_skip==True:
        return None

    # 将能量信息转换为 PyTorch 张量
    if exchange_skip==True:
...
def process_subgraph(subgraph,k_hop_attention,opt_dis):
    ...
    pyg_graph = networkx_to_pyg_graph(subgraph,k_hop_attention,opt_dis)
    return pyg_graph
...
class GAT(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch, li_ni_indices, k_neighbors, u_nb, v_nb, num_nodes):
        ...
        x_f = self.conv_f(x2, edge_index, edge_attr)  
        x_f = F.relu(x_f)
        # 计算每个图的起始索引
...
def main(split_ratio,num_epochs,early_stopping_patience,batch_size,lr_rate,weight_decay_value,seed,k_hop_attention,gat_heads,opt_dis,db_path,k_nn,gat_eps,debug):
    ...
    print("Create train graphs...")
    train_dataset = convert_graphs(train_graphs,k_hop_attention,opt_dis)
    print("\nCreate test graphs...")
    test_dataset = convert_graphs(test_graphs,k_hop_attention,opt_dis)
    print()
    ...
    model = GAT(num_node_features,gat_heads)
...
if __name__ == "__main__":
    ...
    db_dir='/hpc/data/home/spst/xiety/GNN_GAT_train/test_5_correct_dataset/'
    split_ratio=0.9
    ...
    opt_dis=True   #True for optimized str train(DFT related)
    
    #db_path = f'{db_dir}/subgraphs_k_neighbor_{k_hop_attention}_gnn_53d_feature_train.db'
    db_path = f'{db_dir}/subgraphs_k_neighbor_5_gnn_53d_feature_train.db'
    ...
    if debug==1:
        ...
        num_epochs=3
    main(split_ratio,num_epochs,early_stopping_patience,batch_size,lr_rate,weight_decay_value,seed,k_hop_attention,gat_heads,opt_dis,db_path,k_nn,gat_eps,debug)
